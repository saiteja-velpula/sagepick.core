...
async def test_movie_discovery_job_handles_empty(monkeypatch):
    ...
    class FakeTMDB:
        async def discover_movies(self, search_params):
            return types.SimpleNamespace(movies=[], pagination=types.SimpleNamespace(total_pages=1))

        async def close(self):
            return None

    async def fake_process_movie_batch(*args, **kwargs):
        return types.SimpleNamespace(attempted=0, succeeded=0, failed=0, skipped_locked=0)
    ...
    monkeypatch.setattr("app.core.job_execution.job_execution_manager.register", fake_register)
    ...
    monkeypatch.setattr("app.crud.movie_discovery_state.get_current_page", fake_get_current_page)
    monkeypatch.setattr("app.utils.movie_processor.process_movie_batch", fake_process_movie_batch)
